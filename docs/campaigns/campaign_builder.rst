...
.. list-table::
   ...
   * - **Send Email**
     - Sends an Email to the selected Contact. The action includes a **Repeatable email** toggle: set to **Yes** to allow sending the Email to the same Contact multiple times within the Campaign; set to **No** to send the Email only once across all sources. If the Contact has already received a non-repeatable Email from another source or the current Campaign, they aren't sent the Email again and the Contact progresses through the Campaign. If the selected Email has **Send to unsubscribed contacts** enabled, Mautic displays this information in the action configuration.
   * - **Send Email to User** 
