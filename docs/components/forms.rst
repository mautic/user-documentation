...
#. Click the Add a new field dropdown and select tye type of field you wish to use. Available fields include:
...
- **Save result** - When No, Mautic won't save the data entered in the Form. When Yes, the submissions  are accessible in the Form submission results. If a field is mapped to a Contact field, Mautic still saves the data to that field.
- **Default value** - This allows the marketer to provide a default value for a field. This can be used with hidden fields, or when you expect the visitor to enter a certain value. The Contact can change the default value when they complete the Form if the field is visible.
